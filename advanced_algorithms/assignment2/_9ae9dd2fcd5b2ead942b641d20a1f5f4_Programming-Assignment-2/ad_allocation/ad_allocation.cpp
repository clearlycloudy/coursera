...
public:
    ...
    static bool init( simplex & s, std::vector< std::vector< double > > a, std::vector< double > b, std::vector< double > c ){
    ...
    // static double compute_objective_value( simplex & s ){
    // 	double d = 0;
    ...
    static std::pair<int, std::pair<int,int> > compute_next_pivot( simplex & s ){
	int pivot_col = -1;
	int pivot_row = -1;
	// cout << "# pivot" << endl;
    ...
    static std::pair<int, std::pair<int,int> > compute_next_pivot_aux( simplex & s ){
    ...
    static bool gaussian_elimination( simplex & s, std::pair<int,int> pivot ){
	// cout << "# eliminate" << endl;
    ...
    static bool gaussian_elimination_aux( simplex & s, std::pair<int,int> pivot ){
    ...
    static std::pair< ans, std::vector<double> > solve( simplex & s ){
	...
	for( size_t i = 0; i < s._n; ++i ){
	    ...
	    s._map_basic_to_row[ s._m + i ] = i;
	}

	s._m += s._n;

	//introduce artificial variable if necessary for phase 1 to get a basic feasible solution
	vector<int> row_aux; //which rows to add the auxilliary variables
	for( size_t i = 0; i < s._n; ++i ){
	    if( s._b[i] < 0 ){
	        row_aux.push_back(i);
		++s._num_aux;
	...
	for( size_t j = 0; j < row_aux.size(); ++j ){
	    ...
	    s._c_aux[c] = -1.0; //set aux obj function coeff to be maximized
	    col_aux.insert( c );
	    s._map_row_to_basic[r] = c;

	    s._map_basic_to_row.erase( old_basic );
	    s._map_basic_to_row[ c ] = r;
	}
	...
	if( s._num_aux > 0 ){
		...
		if( ret.first == 0 ){ //basic feasible solution found
		    ...
		    return { ans::NONE, {} }; //no solution
		} else if( ret.first == 3 ){
		    return { ans::UNBOUNDED, {} };
	...
	return { ans::BOUNDED, v };
    }
    static void print( simplex & s ){
    ...
    static void print_aux( simplex & s ){
...
pair<int, vector<double>> solve_diet_problem(
    ...
    simplex s;
    simplex::init( s, A, b, c );

    simplex::ans a;
    vector<double> v;
    std::tie( a, v ) = simplex::solve( s );
    switch( a ){
    case simplex::ans::BOUNDED:
	return {0, v};
    case simplex::ans::UNBOUNDED:
	return {1, v};
    case simplex::ans::NONE:
	return {-1, v};
    }
}
